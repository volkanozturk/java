...
public class MultiplePredicatesToStreamFilter {
	...
	public static void main(String[] args) {
		...
		final String myEvenIntegersOrGreaterThan5WithReducedPredicates = myIntegerList.stream().filter(myPredicatesWithOr).map(Object::toString).collect(Collectors.joining(", "));

		final String myEvenIntegersOrGreaterThan5WithPredicateOperators = myIntegerList.stream().filter(isEven.or(isGreaterThan5)).map(Object::toString).collect(Collectors.joining(", "));

		final String myEvenIntegersAndGreaterThan5WithReducedPredicates = myIntegerList.stream().filter(myPredicatesWithAnd).map(Object::toString).collect(Collectors.joining(", "));

		final String myEvenIntegersAndGreaterThan5WithPredicateOperators = myIntegerList.stream().filter(isEven.and(isGreaterThan5)).map(Object::toString).collect(Collectors.joining(", "));

		final String myOddIntegersAndSmallerThanOrEqualTo5WithPredicateOperators = myIntegerList.stream().filter(isEven.negate().and(isGreaterThan5.negate())).map(Object::toString).collect(Collectors.joining(", "));
